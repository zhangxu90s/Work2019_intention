...
from keras.layers import Embedding, Input, Lambda,LSTM,add,dot,Dense,Activation,subtract,Add,multiply,concatenate,merge,Dropout,BatchNormalization,multiply
...
def base_network(input_shape):
    ...
    p = LSTM(200, return_sequences=True, dropout=0.1, recurrent_dropout=0.1,name='f_input')(p)
...
def siamese_model():
    ...
    adm = Adam(lr=0.002)
    model.compile(loss=loss, optimizer=adm, metrics=['accuracy',precision, recall, f1_score])
    return model
...
def train():
    ...
    print("Test best model char=loss: %.4f, accuracy:%.4f, precision:%.4f,recall: %.4f, f1_score:%.4f" % (loss, accuracy, precision, recall, f1_score))
